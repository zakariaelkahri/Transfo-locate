...
def get_of_mat(OF: OFRequest):
    ...
    for i in resources_mfgmat["$resources"] :
        if i["ALLQTY"] > 0 :
            ...
            f"?representation=STOALL.$lookup&count=1000&where=VCRNUM%20eq%20%27{OF.OF}%27 and VCRLIN eq {i["MFGLIN"]} and VCRTYP eq 10 and ITMREF eq %27{i["ITMREF"]}%27",
            auth=(settings.SAGE_API_USER, settings.SAGE_API_PASSWORD)
            )
            ...
            for j in resources_stoall["$resources"] :
                ...
                f"{settings.SAGE_API_URL_STOCK}"
                f"?representation=STOCK.$lookup&count=1000&where=STOFCY%20eq%20%27SIEGE%27 and STOCOU eq {j["STOCOU"]}",
                auth=(settings.SAGE_API_USER, settings.SAGE_API_PASSWORD)
                )
